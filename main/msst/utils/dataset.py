...
def ns_to_cfgdict(ns):
    if isinstance(ns, argparse.Namespace):
        return ConfigDict({k: ns_to_cfgdict(v) for k, v in vars(ns).items()})
    elif isinstance(ns, dict):
        return ConfigDict({k: ns_to_cfgdict(v) for k, v in ns.items()})
    else:
        return ns
...
class MSSDataset(torch.utils.data.Dataset):
    def __init__(self, config, data_path, metadata_path="metadata.pkl", dataset_type=1, batch_size=None, verbose=True):
        ...
        self.config = ns_to_cfgdict(config)

        # Augmentation block
        self.aug = False
        if 'augmentations' in config:
            if self.config['augmentations'].enable is True:
        ...
        self.do_chunks = self.config.get('precompute_chunks', False) and float(self.min_mean_abs) > 0
    ...
    def __len__(self):
        ...
        # For dataset types 1, 2, 3: samples are random (index is ignored in __getitem__)
        # Return arbitrarily large number - actual epoch length controlled by trainer's limit_train_batches
        if self.dataset_type in [1, 2, 3]:
            return 10000000

        # For dataset_type 4: Calculate total number of chunks based on audio lengths and chunk_size
        # # metadata format: [[track_path, num_samples], ...]
        # total_chunks = 0
        # for track_info in self.metadata:
        #     if len(track_info) >= 2:
        #         num_samples = track_info[1]
        #         # Calculate how many chunks fit in this track
        #         num_chunks = max(1, num_samples // self.chunk_size)
        #         total_chunks += num_chunks

        # return total_chunks if total_chunks > 0 else len(self.metadata)
    ...
    def _precompute_random_chunks(self):
        ...
        target_count = getattr(self.config, 'num_precompute_chunks', self.config.num_steps * self.batch_size * self.config.num_epochs)
        chunks_metadata = []
        ...
        with tqdm(total=target_count, desc='Progress good chunks') as pbar:
            while len(chunks_metadata) < target_count:
                batch_size = getattr(self.config, 'precompute_batch_for_chunks', 500)
                tasks = []
...
class MSSDataset_Tracks(MSSDataset):
    """
    Subclass of MSSDataset that outputs data in the same format as MUSDB_Aug_Repitch_Tracks_Dataset.
    Returns (selected_stem, one_hot_vector, stem_data) instead of (res, mix).
    """

    def __init__(self, config, data_path, metadata_path="metadata.pkl", dataset_type=1, batch_size=None, verbose=True):
        super().__init__(config, data_path, metadata_path, dataset_type, batch_size, verbose)
        # Get num_channels and sample_rate from config
        self.num_channels = config.audio.num_channels if hasattr(config.audio, 'num_channels') else 2
        self.sample_rate = config.audio.sample_rate if hasattr(config.audio, 'sample_rate') else 44100

    def __getitem__(self, index):
        # Load data using parent's loading methods
        if self.dataset_type == 5:
            track_path, offset = self.chunks_metadata[index]
            res = self._load_chunk_by_offset(track_path, offset)
        elif self.dataset_type in [1, 2, 3]:
        ...
        if self.aug:
            if 'mp3_compression_on_mixture' in self.config['augmentations']:
                ...
                mix_conv = mix.cpu().numpy().astype(np.float32)
                required_shape = mix_conv.shape
                mix = apply_aug(samples=mix_conv, sample_rate=self.sample_rate)  # ✅ FIXED: Use self.sample_rate
                # Sometimes it gives longer audio (so we cut)
                if mix.shape != required_shape:
                    mix = mix[..., :required_shape[-1]]
                mix = torch.tensor(mix, dtype=torch.float32)
        ...
        # Convert to desired number of channels if needed
        if self.num_channels == 1 and res.shape[1] == 2:
            # Randomly select left or right channel for more diversity
            channel_idx = random.randint(0, 1)
            res = res[:, channel_idx:channel_idx+1, :]  # (num_instruments, 2, chunk_size) -> (num_instruments, 1, chunk_size)

        # res shape: (num_instruments, channels, chunk_size)
        # Randomly select a stem
        stem_index = random.randint(0, len(self.instruments) - 1)
        selected_stem = res[stem_index]

        # Create a one-hot vector for the class index
        one_hot_vector = np.zeros(len(self.instruments), dtype=np.float32)
        one_hot_vector[stem_index] = 1.0
        one_hot_vector = torch.tensor(one_hot_vector, dtype=torch.float32)

        # Return in MUSDB_Aug_Repitch_Tracks_Dataset format
        return selected_stem, one_hot_vector, res

    def augm_data(self, source, instr):
        """Override parent augm_data to use self.sample_rate instead of hardcoded 44100"""
        # source.shape = (channels, samples) - can be (1, samples) for mono or (2, samples) for stereo
        source_shape = source.shape
        applied_augs = []
        if 'all' in self.config['augmentations']:
            augs = self.config['augmentations']['all']
        ...
        # Channel shuffle (only has effect on stereo audio, does nothing for mono)
        if 'channel_shuffle' in augs:
            if augs['channel_shuffle'] > 0:
                if random.uniform(0, 1) < augs['channel_shuffle']:
                    if source.shape[0] == 2:  # Only shuffle if stereo
                        source = source[::-1].copy()
                        applied_augs.append('channel_shuffle')
        # Random inverse
        if 'random_inverse' in augs:
